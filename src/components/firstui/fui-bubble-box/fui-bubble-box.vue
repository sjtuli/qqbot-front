<!--本文件由FirstUI授权予上海交在科技有限公司（手机号：1 59  0 07 3 3  6 0 4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-bubble__box-wrap" ref="fui_bb_wrap">
		...
		<view class="fui-bubble__box-inner" ref="fui_bb_el" :style="getBbStyl"
			...
			<view class="fui-bubble__box-list" :style="{background:background,width:width+'rpx'}">
				<view class="fui-bubble__box-item" v-for="(item,index) in items" :key="index"
					...
					<view class="fui-bubble__box-line"
						...
						:style="{background:lineColor}" v-if="index!==items.length-1 && showLine"></view>
				</view>
			</view>
