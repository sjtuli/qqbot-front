<!--本文件由FirstUI授权予上海交在科技有限公司（手机号：15  9 0  0 7  3 360 4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toDate(date, def) {
				if (this.minDate === true || this.minDate === 'true' || !this.minDate) {
					date = def
				}
...
<style scoped>
	...
	/* #ifndef APP-NVUE*/
	.fui-date__picker-indicator::v-deep {
		position: relative;
		border-color: transparent;
	}

	.fui-date__picker-indicator::v-deep::before {
		border-color: #333;
	}

	.fui-date__picker-indicator::v-deep::after {
		border-color: #333;
	}
