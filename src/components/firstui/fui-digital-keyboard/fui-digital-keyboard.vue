<!--本文件由FirstUI授权予上海交在科技有限公司（手机号：1 5 90 07    33  60 4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
		...
		<view class="fui-digital__keyboard-grids">
			<view class="fui-digital__keyboard-left">
				<view class="fui-dk__left-wrap" v-for="(item,index) in items" :key="item.id">
					<view class="fui-dk__grid-left" v-for="(val,idx) in item.keys" :key="idx">
						<text :style="{fontSize:`${index===3 && idx===0? spareSize:40}rpx`}"
							class="fui-digital__keyboard-key"
							:class="{'fui-dk__key-dark':theme==='dark','fui-dk__highlight-dark':theme==='dark' && val,'fui-dk__highlight':theme==='light' && val}"
			...
			<view class="fui-digital__keyboard-right">
				<view class="fui-dk__grid-right">
					<view class="fui-digital__keyboard-key fui-dk__highlight"
						:class="{'fui-dk__key-dark':theme==='dark','fui-dk__highlight-dark':theme==='dark'}"
						@tap.stop="backspace">
