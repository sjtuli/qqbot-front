<!--本文件由FirstUI授权予上海交在科技有限公司（手机号： 159 0 073  3 60    4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-gallery__wrap" :style="{zIndex:zIndex}" :class="[show?'fui-gallery__show':'fui-gallery__hidden']"
		v-if="show || !isNvue" @tap.stop="hideGallery">
		<swiper class="fui-gallery__img-wrap" :indicator-dots="false" @change="change" :current="defActive"
			:autoplay="false" :duration="500">
			<swiper-item class="fui-gallery__swiper-item" v-for="(item,index) in imgUrls" :key="index">
