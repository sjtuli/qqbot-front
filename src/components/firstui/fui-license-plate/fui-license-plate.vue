<!--本文件由FirstUI授权予上海交在科技有限公司（手机号：1 59 0  07   3 36 0 4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
		...
		<view class="fui-license__plate-grids">
			<view class="fui-lp__grid-wrap" v-for="(item,index) in keyList" :key="item.id">
				<view class="fui-lp__grid"
					...
					<text :style="{fontSize:`${index===3 && idx===0? 28:(type===1?32:36)}rpx`}"
						class="fui-license__plate-key"
						:class="{'fui-lp__key-dark':theme==='dark','fui-lp__highlight-dark':theme==='dark' && val,'fui-lp__highlight':theme==='light' && val}"
						@tap.stop="keyClick(index,idx,val)">{{type===1?val.cn:val.en}}</text>
				</view>
				<view class="fui-lp__grid fui-lp__grid-lg" v-if="index===3">
					<view class="fui-license__plate-key"
						:class="{'fui-lp__key-dark':theme==='dark','fui-lp__highlight-dark':theme==='dark','fui-lp__highlight':theme==='light'}"
						@tap.stop="backspace">
