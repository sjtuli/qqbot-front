<!--本文件由FirstUI授权予上海交在科技有限公司（手机号： 159 0   07  3 36 0 4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-panel__wrap"
		...
		<view class="fui-panel__mediabox" :style="{paddingLeft:padding+'rpx',paddingRight:padding+'rpx'}"
			:class="{'fui-mediabox__center':!flexStart,'fui-panel__hover-weex':highlight}" v-for="(item,index) in list"
			...
			<view v-if="bdBorder && index!==list.length-1"
...
<script>
	export default {
		...
		methods: {
			initData(val) {
				val = val || {};
				let list = val.list && [...val.list]
				if (val.list && Array.isArray(list)) {
					this.list = list;
				} else {
					this.list = [val];
				}
...
<style scoped>
	...
	.fui-panel__background {
		background: var(--fui-bg-color, '#fff');
	}
