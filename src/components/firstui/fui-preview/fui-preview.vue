<!--本文件由FirstUI授权予上海交在科技有限公司（手机号：159      00733    604，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-preview__wrap" :class="{'fui-preview__unlined':!isBorder,'fui-preview__background':!background}"
		:style="{'border-top-color':borderColor,'border-bottom-color':borderColor,marginTop:marginTop+'rpx',marginBottom:marginBottom+'rpx'}">
		<view class="fui-preview__hd" :style="{paddingLeft:padding+'rpx',paddingRight:padding+'rpx'}"
			...
			<view class="fui-preview__item">
				<text class="fui-preview__label" :class="{'fui-preview__gray':!labelColor}"
					:style="{color:labelColor,fontSize:hdLabelSize+'rpx'}">{{pvd.label || ''}}</text>
				<text class="fui-preview__value-hd" :class="{'fui-preview__color':!hdValColor}"
					:style="{color:hdValColor,fontSize:hdValSize+'rpx'}">{{pvd.value || ''}}</text>
			</view>
		...
		<view class="fui-preview__bd" v-if="pvd.list && pvd.list.length>0"
			...
			<view class="fui-preview__item" v-for="(item,index) in pvd.list" :key="index">
				<text class="fui-preview__label" :class="{'fui-preview__gray':!labelColor && !item.labelColor}"
					:style="{color:item.labelColor || labelColor,fontSize:bdSize+'rpx'}">{{item.label}}</text>
				<text class="fui-preview__value" :class="{'fui-preview__color':!bdColor && !item.valueColor}"
					:style="{color:item.valueColor || bdColor,fontSize:bdSize+'rpx'}">{{item.value}}</text>
			</view>
		</view>
		<slot></slot>
...
<script>
	export default {
		...
		methods: {
			...
			handleClick(idx) {
				let params = this.pvd.buttons[idx] || {}
				this.$emit('click', {
					index: idx,
					...params
				})
