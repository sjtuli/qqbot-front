<!--本文件由FirstUI授权予上海交在科技有限公司（手机号：    159 00 73 3 60  4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
				...
				<view class="fui-table--inner" :style="{width:totalW+'px'}">
					<view class="fui-table--tr"
						...
						<view class="fui-table--td"
							:class="{'fui-table__border-right':border,'fui-table__border-left':border && index===0,'fui-table__center':align==='center','fui-table__right':align==='right','fui-table--col-fixed':item.fixed && !isNvue}"
							...
							<text class="fui-table--td-text"
								:class="{'fui-text__center':align==='center','fui-text__right':align==='right','fui-td__ellipsis':ellipsis}"
					...
					<view class="fui-table--tr"
						...
						<view class="fui-table--td"
							:class="{'fui-table__border-right':border,'fui-table__border-left':border && idx===0,'fui-table__center':align==='center','fui-table__right':align==='right','fui-table__td-wrap':model.type===3,'fui-table--col-fixed':model.fixed && !isNvue}"
							...
							<template v-if="model.type!==3">
								<image class="fui-table--td-img" :src="item[model.prop] || ''" mode="widthFix"
									v-if="item.type===2"
									:style="{width:model.imgWidth+'rpx',height:model.imgHeight?model.imgHeight+'rpx':'auto'}">
								</image>
								<text class="fui-table--td-text"
									:class="{'fui-text__center':align==='center','fui-text__right':align==='right','fui-td__ellipsis':ellipsis,'fui-td__wrap':!ellipsis}"
									v-else
									:style="{color:model.textColor || textColor,fontSize:(model.textSize || textSize) +'rpx',width:(model.width+divideW)+'px'}">{{item[model.prop] || ''}}</text>
							</template>
							<template v-else>
