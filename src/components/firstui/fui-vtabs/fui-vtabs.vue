<!--本文件由FirstUI授权予上海交在科技有限公司（手机号： 1590 0 7 3    360  4，身份证尾号：E8J88D）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-vtabs__wrap" :style="{width:vtabsW,height:vtabsH}">
		<view :style="{width:tabWidth+'rpx',background:background}">
			<scroll-view :show-scrollbar="false" class="fui-vtabs__scroll-bar" :style="{width:tabWidth+'rpx'}" scroll-y
		...
		<scroll-view :show-scrollbar="false" scroll-y class="fui-vtabs__content-wrap" :scroll-top="contentScrollTop"
			:scroll-with-animation="animation" @scroll="contentScroll">
			...
			initData(vals) {
				if (vals && vals.length > 0) {
					...
					this.$nextTick(() => {
						if (this.linkage) {
							setTimeout(() => {
								this.setDefaultTab(this.activeTab, true);
							}, 50)
						} else {
