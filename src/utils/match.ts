// 匹配有没有中文
export function regZh(str: string) {
  return RegExp(/[\u4E00-\u9FA5]+/).test(str)
}
// 只允许中文
export function regOnlyZh(str: string) {
  return RegExp(/^[\u4E00-\u9FA5]+$/).test(str)
}
// 只允许中文，英文字母，数字
export function regZhEnNum(str: string) {
  return RegExp(/^[\u4E00-\u9FA5A-Za-z0-9]+$/).test(str)
}
// 匹配身份证
export function regIDC(str: string) {
...
// 匹配域名
export function regRealmName(str: string) {
  return RegExp(
    /[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\/.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\/.?/,
  ).test(str)
}
// 匹配密码（密码(以字母开头，长度在6~18之间，只能包含字母、数字和下划线)）
export function regPwd(str: string) {
  return RegExp(/^[a-zA-Z]\w{5,17}$/).test(str)
}
// 日期格式  2019-01-12
export function regDate(str: string) {
  return RegExp(/^\d{4}-\d{1,2}-\d{1,2}/).test(str)
}
// 匹配空白行
export function regWhiteLine(str: string) {
  return RegExp(/\n\s*\r/).test(str)
}
// 匹配正数、负数、和小数
export function regNum1(str: string) {
  return RegExp(/^(\-|\+)?\d+(\.\d+)?$/).test(str)
}
// 匹配零和非零开头的数字
export function regNum2(str: string) {
  return RegExp(/^(0|[1-9][0-9]*)$/).test(str)
}
// 匹配数字
export function regNum3(str: string) {
  return RegExp(/^[0-9]*$/).test(str)
}
// 匹配m-n位的数字
export function regNum4(str: string) {
  return RegExp(/^\d{m,n}$/).test(str)
}
// 长度为3-20的所有字符
export function allByte(str: string) {
  return RegExp(/^.{3,20}$/).test(str)
}

// 匹配9-10位的数字
export function groupNum(str: string) {
  return RegExp(/^\d{9,10}$/).test(str)
}
